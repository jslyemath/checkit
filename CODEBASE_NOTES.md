...
This document is a complete, self-contained reference for the CheckIt codebase. Every class, function, file, and design decision is explained here in enough detail that someone with no access to the source files can answer any question about how the system works.
...
│       └── wrapper/                 SageMath execution harness
...
│   ├── bank.xml                     manifest listing 7 outcomes
...
**`generate(amount, regenerate, images, outcome)` — `checkit generate`:**
Options:
...
3. Calls `b.generate_exercises(regenerate=..., images=..., amount=...)`
...
**`Bank.generate_exercises(self, regenerate=False, images=False, amount=1_000)`**
Iterates `self.outcomes()`, prints a progress message, and calls `o.generate_exercises(...)` for each one.
...
**Imports:** `.exercise.Exercise`, `os`, `json`, `random`, `html.escape`, `.wrapper.sage`
...
Used by the (deprecated) Jupyter dashboard for "fresh preview". Calls `sage(self, preview_json, preview=True, images=True)` to generate 20 seeds, reads them, and returns a list of `Exercise` objects.
...
**`Outcome.generate_exercises(self, regenerate=False, images=False, amount=1_000)`**
- If `regenerate=False`, tries `self.load_exercises()`. If that succeeds (seeds.json exists and is valid), returns early.
- Otherwise calls `sage(self, self.seeds_json_path(), preview=False, images=images, amount=amount)` — this invokes the SageMath subprocess.
...
**`sage(outcome, output_path, preview=True, images=False, amount=1_000, random=False)`**
...
7. Runs: `sage /tmp/xxx/wrapper.sage <generator_path> <output_path> <amount> <random_s> [images]`
...
```
sage wrapper.sage <generator_path> <output_path> <amount> [random|no] [images]
```
...
### Making Images Generatable with PreFigure
...
Current image generation in `wrapper.sage`:
```python
if gen_images:
