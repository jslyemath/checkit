...
- **The clipboard write can be refused, and must not fail silently.** `navigator.clipboard.writeText()` rejects with `NotAllowedError` in some browsers and permission states even on a secure origin inside a click handler — observed on the published site, where the first version of this button did nothing at all and logged nothing. `Outcome.svelte` now falls back to `document.execCommand('copy')` (deprecated, but it needs only the user gesture rather than a permission), and if that fails too it reveals the payload in a readonly textarea with an explanation. Also note the copy state resets whenever the student changes outcome or version: routing is hash-based and never reloads the page, so otherwise a stale "Copy blocked" textarea holding the *previous* exercise follows them to the next one.
